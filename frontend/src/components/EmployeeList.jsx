...
const EmployeeList = () => {
    ...
    let deleteUser = (e)=>{
      ...
      .then(() => {
        setReload(!reload); // Toggle reload to refresh data after deletion
    })
        ...
        <tbody className='text-center text-[15px]'>
          {infoFromDB.map((item,i) => (
            <tr key={item.id}>
              ...
              <td className='border-2 border-green-700'><img src={`http://localhost:4001/backend/Images/${item.image}`} alt="Employee"/>
              </td>
